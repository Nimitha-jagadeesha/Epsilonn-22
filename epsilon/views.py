    ...
    if  request.method == 'POST' and 'skipbutton' == request.POST.get('action'):
        ...
        if answer.lower().strip() == crct_answer.lower():
            ...
            try:
                ...
                x = False
            return render(request, 'epsilon/arena.html', {'question': question,'x':x,'display':display,'qnum':len(request.user.score.picked), 'is_staff':request.user.is_staff})
    if question == None:
         return render(request, 'epsilon/arena.html', {'done': True, 'is_staff':request.user.is_staff})
    return render(request, 'epsilon/arena.html', {'question': question,'x':x,'display':display, 'qnum':len(request.user.score.picked), 'is_staff':request.user.is_staff})